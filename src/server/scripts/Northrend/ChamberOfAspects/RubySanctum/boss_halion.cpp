...
public:
    ...
    struct boss_halion_realAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
			...
			// Halion detecta a un jugador a una distancia de 60 y dice la primera frase
			...
			// Halion detecta a un jugador a una distancia de 20, pasa al estado 10 y lo pone en combate
        ...
        void JustDied(Unit* pKiller)
        {
			...
			// Controla que Halion de las sombras este muerto
        ...
        void KilledUnit(Unit* pVictim)
        {
			...
			if(!UpdateVictim() && phase == 3){// DEBERIAMOS PONERLO CUANDO ENTRA EN EVADEMODE, ENTONCES NO ESTARIA ESPERANDO A MATAR AL ULTIMO PAISANO.
				if (Creature* hSombras = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_TWILIGHT))){
					hSombras->AddAura(SPELL_BERSERK,hSombras);
					instance->SetData(TYPE_HALION_EVENT, FAIL); // esto lo tendriamos que comentar para que no deje a la gente abajo, supuestamente si entra en berserk todos deber�an morir.
        		} // que pasa si muere el de abajo pero el de arriba no?
        		// en la muerte tambien se puede checkear si los 2 halion estan muertos y de no ser as� que no sea loteable.
			}	
			
        }
		
		// Halion entra en combate
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            switch (getStage())
            {
				// Fase inicial Halion esta invisible y comienza un terremoto
                case 10: //PHASE INTRO
                    ...
                    break;
				// Inicio del boss ya realiza ataques
                case 0: //PHASE 1 PHYSICAL REALM
					...
					// Cuando el boss llega al 70% de vida
                ...
                case 1: // Switch to phase 2
                    {
						...
						// El boss se mueve al centro
                        StartMovement(0);
							// Invoca al npc controlador en el caso de que no este (el cual se encargara de tratar el tema de la Corporaidad)
                ...
                case 2:
                    if (MovementStarted)
                        return;
					// Halion castea el portal para pasar al reino del Crepusculo
                    DoCast(me, SPELL_SUMMON_TWILIGHT_PORTAL);
                    setStage(3);
					// Pone el Portal en la phase 31
					...
					// Pone el circulo de fuego violeta para que la gente no salga del circulo en el reino crepuscular en la phase 65535
					if (GameObject* pGoRing = instance->instance->GetGameObject(instance->GetData64(GO_FLAME_CREP)))
					pGoRing->SetPhaseMask(65535,true);
					//Ahora se queman los arboles < script HALION>
                ...
                case 3:
                    if (me->IsNonMeleeSpellCasted(false))
                        return;
					// Halion castea una spell para dejarlo invisible
                ...
                case 4:
                    if (!me->IsNonMeleeSpellCasted(false))
                    {
						...
						// Invoca a halion del reino crepuscular en el caso de que no este invocado ya
                    ...
                    break;
                case 5: // Halion espera a que se finalize la batalla en el crepusculo (Fase 2)
...
//                            instance->SetData(TYPE_HALION_EVENT, SPECIAL);
                            ...
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
							// Pone a Halion del mundo real con la mitad de la vida (Porque sera con la vida que tiene que estar cuando inicie la phase 3)
...
//                    DoCast(SPELL_TWILIGHT_DIVISION);
                ...
                case 7:
                    ...
                    if (me->getVictim()->GetTypeId() != TYPEID_PLAYER)
                        return;
                    SetCombatMovement(true);
					// Colocar un comprobador al getVictim??
                    me->GetMotionMaster()->MoveChase(me->getVictim());
                    setStage(8);
                    break;

                case 8: //Ataques de la phase 3
                    if (m_uiFlameTimer <= uiDiff)
                    {
...
public:
    ...
    struct boss_halion_twilightAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			// Pone la vida del halion de la sombras como la vida del halion real
            if (Creature* pReal = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_REAL)))
                if (pReal->isAlive())
                    me->SetHealth(pReal->GetHealth());
			// Le añade la phase al halion de las sombras
		...
		// Cuando Halion detecta movimiento a 20, elimina su flag de no atacable, pasa al estado 1 y añada la spell de TWILIGHT_PRECISION
        void MoveInLineOfSight(Unit* pWho)
        {
            if (!instance) return;
            ...
            if ( !intro && pWho->IsWithinDistInMap(me, 20.0f))
            {
                ...
                intro = true;
                AttackStart(pWho);
				// Pasa al estado de los spells en combate
                setStage(1);
                DoCast(SPELL_TWILIGHT_PRECISION);
				// Pone la vida del halion de la sombras como la vida del halion real
                if (Creature* pReal = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_REAL)))
		...
		// Muerte de Halion Sombras
        void JustDied(Unit* pKiller)
        {
            if (!instance)
                return;
            DoScriptText(-1666104,me);
			// Elimina la spells de la segunda phase a todos los jugadores
            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_TWILIGHT_ENTER);
			
			// Cierra los portales del reino crepuscular
            if (GameObject* pGoPortal = me->FindNearestGameObject(GO_HALION_PORTAL_1, 50.0f))
                   pGoPortal->Delete();
            if (GameObject* pGoPortal = me->FindNearestGameObject(GO_HALION_PORTAL_2, 50.0f))
                   pGoPortal->Delete();
			...
			// Comprueba si halion real esta muerto
            if (Creature* pReal = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_REAL)))
                if (!pReal->isAlive())
                {
                    instance->SetData(TYPE_HALION, DONE);
			...
			// Elimina el cuerpo del halino sombras
        ...
        void KilledUnit(Unit* pVictim)
        {
			...
			// Halion sombras no tiene a quien atacar, se mata a si mismo el evento es fallido y pone al halion real en Rage
			if(!UpdateVictim()){
				me->Kill(me);
				instance->SetData(TYPE_HALION_EVENT, FAIL);
				ScriptedAI::EnterEvadeMode();
				if(phase == 3){
					if (Creature* hReal = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_REAL))){
						hReal->AddAura(SPELL_BERSERK,hReal);
		...
		// Cuando entra en combate elimina el flag de no atacable y añade la spells de daño en el tiempo
        void EnterCombat(Unit* pWho)
        {
            if (!instance)
                return;
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            {
				// En el caso de que alguien se quede en la phase de sombras y ataquen al halion sombras cuando el evento no se ha iniciado
                if (Creature* pReal = me->GetMap()->GetCreature(instance->GetData64(NPC_HALION_REAL)))
                    if (!pReal->isAlive())
                        pReal->Respawn();
...
//                DoCast(SPELL_TWILIGHT_DIVISION);
					...
					// Cuando halion esta el 50% de la vida pasa a la fase 3
                ...
                case 2:           //To two realms
                    ...
                    DoScriptText(-1666109,me);
					// Invoca el portal para poder volver al halion real
...
public:
    ...
    struct mob_halion_orbAI : public ScriptedAI
    {
        ...
        void StartMovement(uint32 id)
        {
            ...
            nextPoint = id;
            float x = SpawnLoc[id].x;
			float y = SpawnLoc[id].y;
            ...
            if (focus = me->GetMap()->GetCreature(instance->GetData64(NPC_ORB_ROTATION_FOCUS)))
                ...
                me->DespawnOrUnsummon();
            me->GetMotionMaster()->Clear();
            me->GetMotionMaster()->MovePoint(id, x, y, 73);
        }
