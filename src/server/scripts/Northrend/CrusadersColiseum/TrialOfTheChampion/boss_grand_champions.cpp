...
struct npc_mounted_championAI : ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (me->isAttackReady() && me->IsWithinMeleeRange(target))
        {
...
public:
    ...
    struct boss_mage_toc5AI : public npc_mounted_championAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoCastVictim(SPELL_FIREBALL); //ojito con esto que puede crashear aqui por no estar testeado en DoCastVictim si existe el me->getvictim y demas.
                fireBallTimer = 2600;
    ...
    struct boss_hunter_toc5AI : public npc_mounted_championAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(me->GetDistance(target) >= 30.0f)
                me->GetMotionMaster()->MoveChase(target, 25.0f);
			}
            ...
            if (me->isAttackReady())
            {
                ...
                {
                    DoCast(target, SPELL_LIGHTNING_ARROWS_DAMAGE, true);
