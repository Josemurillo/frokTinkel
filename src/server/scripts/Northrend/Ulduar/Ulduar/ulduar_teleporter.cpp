    ...
    public:
        ...
        bool OnGossipSelect(Player* player, GameObject* /*gameObject*/, uint32 sender, uint32 action)
        {
            ...
            switch (action)
            {
				...
				// Mimiron
				case IMAGINATION:
					player->TeleportTo(603, 2531.00f, 2569.46f, 412.30f, 0.03f);
					player->CLOSE_GOSSIP_MENU();
                    break;
            }
        ...
        bool OnGossipHello(Player* player, GameObject* gameObject)
        {
            ...
            if (InstanceScript* instance = gameObject->GetInstanceScript())
            {
				...
				// Modificado el boss a matar ya que Kologar esta eliminado
                if (instance->GetBossState(BOSS_XT002) == DONE)
				...
				// Ir a mimiron tren no funciona
				if(instance->GetBossState(BOSS_AURIAYA) == DONE && instance->GetBossState(BOSS_HODIR) == DONE && instance->GetBossState(BOSS_THORIM) == DONE){
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Teleport to the Spark of Imagination", GOSSIP_SENDER_MAIN, IMAGINATION);
				}
            }
