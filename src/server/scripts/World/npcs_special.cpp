...
public: 
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (player->HasAchieved(ACHI_PONY_UP)){
			...
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Visit a auction house.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_AUCTION);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        switch (action)
        {
			case GOSSIP_ACTION_AUCTION:
				...
				creature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_AUCTIONEER);
				break;
