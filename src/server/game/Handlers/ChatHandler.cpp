    ...
    if (!ignoreChecks)
    {
        ...
        if (msg.empty())
         ...
         if(sWorld->getBoolConfig(CONFIG_WORD_FILTER_ENABLE))
         {
             ...
             { 
				 if (sender)
					 if (!sender->m_sentMsgCache.empty()){
				 int charguid = sender->GetGUID();
				 sLog->outDebug(LOG_FILTER_SPAM, "guid: %u mensaje: %s",charguid,sender->m_sentMsgCache.c_str());
				 //CharacterDatabase.PExecute("Insert into spam (guid,mensaje,time) values ('%u','%s',now())",charguid,sender->m_sentMsgCache.c_str());
					 }
