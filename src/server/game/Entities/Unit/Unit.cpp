...
Unit::~Unit()
{
	...
	{
		sLog->outFatal(LOG_FILTER_GENERAL,"Salida antes de caida unit: %u",GetGUID());
		m_sharedVision.clear(); //posible reparacion del crash este.
	}
    ASSERT(m_sharedVision.empty());
		if (!m_Controlled.empty())
	{
		sLog->outFatal(LOG_FILTER_GENERAL,"Salida antes de caida unit: %u",GetGUID());
		m_Controlled.clear(); //posible reparacion del crash este.
	}
...
{
    if (m_sharedVision.empty() && !((Creature*)this)->isPet()) //solve crash original de la web: if (m_sharedVision.empty() && GetTypeId() == TYPEID_UNIT && !((Creature*)this)->isPet() )
    {
        setActive(true);
        SetWorldObject(true);
    }
...
void Unit::RemovePlayerFromVision(Player* player)
{
    ...
    if (m_sharedVision.empty() && !((Creature*)this)->isPet()) //posible reparacion para el assert del unit.
    {
        setActive(false);
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        Dismount();
	if ((type == CHARM_TYPE_VEHICLE) && !IsVehicle()) // or (type == CHARM_TYPE_VEHICLE) != IsVehicle()
		return false;
