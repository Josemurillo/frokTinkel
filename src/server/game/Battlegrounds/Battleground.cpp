...
void Battleground::EndBattleground(uint32 winner)
{
	...
	uint32 checkporhora = 3;
	//checkporhora = sWorld->getIntConfig(CONFIG_ARENA_PARTIDAS_POR_HORA); 
	std::string ip1;
	std::string ip2;
	uint32 ganador=0;
	uint32 perdedor=0;
    ...
    if (isArena() && isRated())
    {
		...
		ganador = winner_arena_team->GetId();
		perdedor = loser_arena_team->GetId();
		SQLTransaction trans = CharacterDatabase.BeginTransaction();
		PreparedStatement* insertar = CharacterDatabase.GetPreparedStatement(CHAR_INS_ARENA_CHECKFARM);
		insertar->setUInt32(0, ganador);
		insertar->setUInt32(1, perdedor);
		trans->Append(insertar);
		CharacterDatabase.CommitTransaction(trans);

		stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ARENA_CHECKFARM);
		stmt->setUInt32(0, ganador);
		stmt->setUInt32(1, perdedor);
		result = CharacterDatabase.Query(stmt);
		if (result){
		arenafarmcheck = (*result)[0].GetUInt64();
		sLog->outDebug(LOG_FILTER_ARENAS,"Salida de arenafarmcheck %u, equipo1:%u , equipo2:%u ",arenafarmcheck,ganador,perdedor);
		}
		else
		{
			sLog->outDebug(LOG_FILTER_ARENAS,"Salida de arenafarmcheck sin resultado, equipo1:%u , equipo2:%u ",ganador,perdedor);
		}
		if (arenafarmcheck>=checkporhora) //primer checkeo, si hacen contra el mismo equipo muchas veces.
			{
				puntuar = false;
			}
		//2º checkeo si los weys intetan farmear contra la misma ip.
		if (puntuar == true)
		{
			for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
			if (Player* player1 = ObjectAccessor::FindPlayer(itr->first))
				 {
				if( (player1->GetArenaTeamId(m_ArenaType == 5 ? 2 : m_ArenaType == 3)) == ganador)
				{
				ip1 =  player1->GetSession()->GetRemoteAddress();
				for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
				if (Player* player2 = ObjectAccessor::FindPlayer(itr->first))
					if ( (player2->GetArenaTeamId(m_ArenaType == 5 ? 2 : m_ArenaType == 3)) != ganador)
					{
						ip2 = player2->GetSession()->GetRemoteAddress();

					sLog->outDebug(LOG_FILTER_ARENAS,"Salida de ips: ip1: %s , player1: %s , ip2: %s, player2: %s  ",ip1.c_str(),player1->GetName(),ip2.c_str(),player2->GetName());

					if (ip1==ip2)
					{
					sLog->outDebug(LOG_FILTER_ARENAS,"no puntua");
					puntuar = false;
					}
					}
				}
		...
		//3er checkeo para tiempo.
		time_t tiempo = time(NULL);
		tm* aTm = localtime(&tiempo);

		if(aTm->tm_hour>7 && aTm->tm_hour<12 )
		{
			puntuar=false;
			sLog->outDebug(LOG_FILTER_ARENAS,"no puntua por horario,%d",aTm->tm_hour);
		}

        if (winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team && puntuar)
        {
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (itr->second.OfflineRemoveTime)
        {
            ...
            if (isArena() && isRated() && winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team  && puntuar)
            {
                if (team == winner)
