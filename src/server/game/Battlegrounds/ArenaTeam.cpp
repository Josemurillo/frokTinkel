...
bool ArenaTeam::Arenacheck(uint64 player, uint8 type, bool notificar)
{
	if (!player && !type)
		return true;
PreparedStatement* selec = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ARENATIME);
selec->setUInt32(0, GUID_LOPART(player));
selec->setUInt32(1, type);
PreparedQueryResult checkeo = CharacterDatabase.Query(selec);
Player * entrando = ObjectAccessor::FindPlayer(player);
if (checkeo) 
	{
		if (entrando)
		{	
		Field* fields = checkeo->Fetch();
		uint64 fecha = fields[0].GetUInt64();
		ChatHandler(entrando).PSendSysMessage("No puedes unirte a un equipo hasta dentro de %u horas.", fecha);
		}
		return true;
	}
else
{
	if (entrando && notificar)
	ChatHandler(entrando).PSendSysMessage("RECUERDA QUE SOLO PUEDES UNIRTE A UN EQUIPO DE ARENA CADA 48 HORAS.");
	return false;
}
...
bool ArenaTeam::Create(uint64 captainGuid, uint8 type, std::string teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
	...
	if(Arenacheck(captainGuid,type,false))
		return false;
...
/*		PreparedStatement* selec = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ARENATIME);
		selec->setUInt32(0, GUID_LOPART(playerGuid));
		selec->setUInt32(1, GetType());
		PreparedQueryResult checkeo = CharacterDatabase.Query(selec);
	...
	if (checkeo)
	{
		//Esto todo no vale para nada ya que la creacion se hace arriba no aqui.
		//Field* fields = checkeo->Fetch();
		//if (player)
		//ChatHandler(player).PSendSysMessage("No te puedes unir a arenas hasta\"%s\" .",fields[0].GetCString());
		return false;
	}*/
	if(Arenacheck(playerGuid,GetType(),true))
	return false;
	else
	{
		PreparedStatement* insertar = CharacterDatabase.GetPreparedStatement(CHAR_INS_ARENATIME);
		insertar->setUInt32(0, GUID_LOPART(playerGuid));
		insertar->setUInt32(1, GetId());
		insertar->setUInt32(2, GetType());
		CharacterDatabase.Execute(insertar);
		//if(player) // esto no vale para nada ya que la creacion la hace arriba aqui solo es para addteam.
		//ChatHandler(player).PSendSysMessage("RECUERDA QUE SOLO PUEDES ENTRAR UNA VEZ POR SEMANA A UN EQUIPO DE ARENAS.");
...
// 20130815 - WoW Ryu - No permitir comprar un item si no se cuenta también con MMR suficiente.
//						Obtener MMR a partir de Guid del miembro de un equipo.
uint32 ArenaTeam::GetMemberMMR(uint32 PlayerGuid) const
{
	if (!PlayerGuid)
		return 0;
...
void ArenaTeam::MemberLost(Player* player, uint32 againstMatchmakerRating, int32 MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        if (itr->Guid == player->GetGUID())
        {
            ...
            // 20130831 - WoW Ryu - Calcular el cambio de MMR individualmente para cada integrante del equipo.
			int32 mmr = GetMemberMMR(itr->Guid);
			mod = GetRatingMod(mmr, againstMatchmakerRating, false);
...
void ArenaTeam::MemberWon(Player* player, uint32 againstMatchmakerRating, int32 MatchmakerRatingChange)
{
    ...
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        if (itr->Guid == player->GetGUID())
        {
            ...
            // 20130831 - WoW Ryu - Calcular el cambio de MMR individualmente para cada integrante del equipo.
			int32 mmr = GetMemberMMR(itr->Guid);
			mod = GetRatingMod(mmr, againstMatchmakerRating, true);
            itr->ModifyMatchmakerRating(mod, GetSlot());
