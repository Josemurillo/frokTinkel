...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(CHAR_INS_PET_AURA, "INSERT IGNORE INTO pet_aura (guid, caster_guid, spell, effect_mask, recalculate_mask, stackcount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxduration, remaintime, remaincharges) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
	...
	PREPARE_STATEMENT(CHAR_SEL_ARENA_CHECKFARM,"SELECT COUNT(ganador) FROM arenas WHERE ganador=? AND perdedor=? AND TIMESTAMPDIFF(HOUR,timestamp,now())<2",CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_INS_ARENA_CHECKFARM,"INSERT INTO arenas (ganador, perdedor, timestamp) VALUES (?,?,now())",CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_INS_ARENATIME,"INSERT INTO arenatime (guid,team,tipo,timestamp) VALUES (?,?,?,now())",CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_ARENATIME,"SELECT 48 - TIMESTAMPDIFF(HOUR,timestamp,now()) FROM arenatime WHERE guid= ? AND tipo= ? and TIMESTAMPDIFF(HOUR,timestamp,now())<48",CONNECTION_SYNCH);
}
